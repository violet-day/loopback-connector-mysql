...
MySQL.prototype._buildProjection = function (model, projection) {
  var cs = [];
  Object.keys(projection).forEach(function (key) {
    var keyEscaped = self.columnEscaped(model, key);
    if (projection[key] && projection[key].constructor.name === 'Object') {
      var aggregateType = Object.keys(projection[key])[0].toUpperCase();
      var sqlCond = aggregateType + '( ' + keyEscaped + ' )' + key;
      cs.push(sqlCond);
    } else {
      cs.push(keyEscaped +' as ' + key);
    }
  });
};

MySQL.prototype.aggregate= function (model, filter, callback) {
  var self = this;
  // Order by id if no order is specified
  filter = filter || {};
  if (!filter.order) {
  ...
  var sql = 'SELECT ' + self._buildProjection(model, filter.projection) + ' FROM ' + this.tableEscaped(model);
  ...
  if (filter.group) {
    sql += ' ' + buildGroup(model,filter.groupBy);
  }
...
//todo:创建分组
function buildGroup(model, groupBy) {
